...
def draw_der(t,i):
        ...
        der= p_der(t,i)
        end_pt = pt+der 
        RM = rotacijska_matrica(t=t, i=i)

       #novi_vrhovi = [np.append(vrh[:3]  @ np.linalg.inv(RM), 1) for vrh in vrhovi]
        novi_vrhovi = [np.append(vrh[:3] @ RM , 1) for vrh in vrhovi]

        nacrtaj_tijelo(ociste, der, pt, novi_vrhovi, top_polig, dcm = True)
...
def rotacijska_matrica(t,i):
    w = p_der(t,i)
    u = np.cross(p_der(t,i)[:3], p_double_der(t,i)[:3])
    v = np.cross(w[:3], u)
    return np.array([[w[0], u[0], v[0]],
                     [w[1], u[1], v[1]],
                     [w[2], u[2], v[2]]])
...
def main():
    ...
    vrhovi = rotiraj(vrhovi, 1,1,1)
    vrhovi = skaliraj_vrhove(vrhovi,0.5)
